...
export function BookingForm({ service }: BookingFormProps) {
  ...
  // Extract user points from nested structure
  const actualUser = userData?.state?.user?.user || userData?.user || userData
  const userPoints = actualUser?.loyaltyPoints || 0
  ...
  const maxGuests = service.capacity || 2
  ...
  const calculateBaseTotal = () => {
    ...
    const price = selectedVariant?.price || service.price || 0
  ...
  const calculateRewardPoints = () => {
    const days = calculateDays()
    const multiplier = selectedVariant?.pointsMultiplier || service.pointsMultiplier || 1
    const basePoints = (service.price || 0) * 0.1 * multiplier
    return Math.floor(basePoints * days * guests)
  }

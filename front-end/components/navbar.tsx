...
export default function Navbar() {
  ...
  // Clean white background like in the screenshot
  const navbarBg = "bg-white"
  const textColor = "text-gray-800"
  const isDashboard = pathname === "/dashboard"
      ...
      <div className="container mx-auto px-4 md:px-6 py-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center">
            ...
            <span className={`ml-3 font-medium ${textColor}`}>Kuriftu Rewards</span>
          </Link>

          {/* Desktop Navigation - right aligned with clean spacing like in screenshot */}
          <nav className="hidden md:flex items-center space-x-8">
            <Link href="/membership" className={`font-medium hover:text-gray-600 transition-colors ${textColor}`}>
              Membership Tiers
            </Link>
            <Link href="/services" className={`font-medium hover:text-gray-600 transition-colors ${textColor}`}>
              Services
            </Link>
            <Link href="/ai-experience" className={`font-medium hover:text-gray-600 transition-colors ${textColor}`}>
              AI Experience
            </Link>
            <Link href="/sustainability" className={`font-medium hover:text-gray-600 transition-colors ${textColor}`}>
              Sustainability
            </Link>

            {user ? (
              <>
                <Link href="/dashboard" className={`font-medium hover:text-gray-600 transition-colors ${textColor}`}>
                  Dashboard
                </Link>
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" onClick={handleLogout} className="h-8 rounded-full">
                    Logout
                  </Button>
                </div>
              </>
              ...
              <Link
                onClick={() => setIsAuthModalOpen(true)}
                href=""
                className="bg-gray-800 hover:bg-gray-700 text-white font-medium py-2 px-4 rounded-full transition-colors"
              >
                Join Now
              </Link>
            )}
        ...
        <div className="md:hidden bg-white">
          <div className="container mx-auto px-4 py-4">
            <nav className="flex flex-col space-y-4">
              <Link
                href="/membership"
                className="font-medium text-gray-800 hover:text-gray-600 transition-colors py-2"
              ...
              {user ? (
                ...
                <Link
                  href=""
                  className="bg-gray-800 hover:bg-gray-700 text-white font-medium py-2 px-4 rounded-full transition-colors inline-block text-center"
                  onClick={() => setIsAuthModalOpen(true)}
                >
                  Join Now
                </Link>
